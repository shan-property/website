...
<section class="flex flex-wrap justify-center gap-y-3 px-1/10">
	...
	<div class="relative basis-full overflow-x-hidden">
		<article class="flex gap-5 overflow-x-scroll pe-1/8">
			{#each data as { node } (node.id)}
				<PropertyCard {...node} />
			{/each}
		</article>
		<div
			class="absolute bottom-0 right-0 top-0 z-10 h-full w-1/10 max-w-[50px] bg-gradient-to-l from-bg2 to-bg2/0"
		></div>
	</div>
</section>
